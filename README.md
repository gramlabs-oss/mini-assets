...
从我们的博客文章了解最近的更新：<https://blog.gramlabs.org/posts/mini-assets-updates-2024-01-11.html>
...
收集自己所需的图片，以尽可能通用的类别命名文件夹隔离存放它们，无需在意图片的大小、名称和格式。假设我找了几张图，有两张“狗熊”和一张“外星人”以及一张“鲸鱼”。我可以按照以下方式存放它们：
...
验证资源是一种易于理解的简单结构，它由清单文件 `Manifest.yml` 和数个图片文件夹组成。您需要生成（生成是自动的）或编辑清单文件，再生成验证资源（日后有修改或增加，依次往复）。

使用名为 `mini-assets-gen` 的程序，将它放在命令行可以方便执行到的位置。这里的 `*-gen` 名称含义就是“生成器（generator）”。在最初的构思中，资源的检索、下载、生成是分步骤进行的，目前只提供了最后一个步骤的工具，也就是本小节所使用的工具。

执行以下命令：

```bash
./mini-assets-gen --prefix=./mini-images
```

对此命令进行一些附加解释：

- 对于 Windows 用户，您执行的文件名是 `mini-asset-gen.exe`。其余的一切，都几乎完全等同（可能存在一些系统差异，例如 Windows 中路径分割符是 `\` 而不是 `/`）。
- 参数 `--prefix` 表示存放图片文件夹们的位置。这里假定所有图片文件夹都放在 `./mini-images` 这个路径中。

如果您执行 `mini-assets-gen` 的位置和图片文件夹放在同一路径，可以忽略 `--prefix` 参数，因为此参数的默认值就是 `.`（当前目录）。
...
1. 在存放图片文件夹的根目录生成了一个名为 `_albums` 的输出目录，此目录内部有和根目录类似的文件树，但具体的图片名不同。
1. 在存放图片文件夹的根目录和 `_albums` 目录内，都生成了一个内容基本等同的 `Manifest.yml` 清单文件。

您需要遵从以下规则：

- 不要对 `_albums` 目录内的任何文件进行变动，包括修改、删除等。
- 修改原始资源目录中的 `Manifest.yml` 文件或对原始资源进行了增减，都需要重新执行命令。
...
```yaml
---
version: 0.2.1 # 生成器版本，无需编辑。
datetime: "2024-01-11T08:49:02Z" # 生成时间，无需编辑。
width: 240 # 生成图片的统一宽度，视资源决定。
height: 160 # 生成图片的同意高度，视资源决定。
include_formats: # 包含的文件格式列表，无需编辑。
  - jpeg
  - png
  - jpg
...
```plaintext
./mini-assets-gen --width=180 --prefix=./mini-images
```
...
您需要安装 magickwand 库，在 Debian 系的发行版中包叫 `libmagickwand-dev`。使用如下命令生成以 ImageMagick 作为图像处理后端的程序：
